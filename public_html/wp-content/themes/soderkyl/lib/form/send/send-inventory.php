...
if ($captcha)
{
    ...
    if (array_key_exists('success', $response) && $response['success'] === true)
    {
		...
		$mail->Subject = 'Ansökan om Tvättstugeinventering från hemsidan';
		$mail->Body = $body;
		...
		$form_recipient_repeater = get_field('inventory_form_recipient_repeater', $_POST['post_id']);
		...
		{
			$mail->AddAddress('info@soderkyl.se', 'Söderkyl Tvättstugeinventeringsformulär');
		}
