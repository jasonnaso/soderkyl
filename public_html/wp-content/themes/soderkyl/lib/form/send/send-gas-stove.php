...
if ($captcha)
{
    ...
    if (array_key_exists('success', $response) && $response['success'] === true)
    {
		...
		$mail->Subject = 'Boka service av gasspisar';
		$mail->Body = $body;
		...
		$form_recipient_repeater = get_field('gas_stove_form_recipient_repeater', $_POST['post_id']);
		...
		{
			$mail->AddAddress('info@soderkyl.se', 'Söderkyl Gasspisformulär');
		}
