...
if ($captcha)
{
    ...
    if (array_key_exists('success', $response) && $response['success'] === true)
    {
		...
		$body .=  'Serviceadress: ' . $_POST['address'] . "\n";
		$body .=  'Produkt eller Produktnr/typ.nr: ' . $_POST['product'] . "\n";
		...
		$mail->Subject = 'Meddelande från hemsidan';
		$mail->Body = $body;
		...
		$form_recipient_repeater = get_field('service_form_recipient_repeater', $_POST['post_id']);
		...
		{
			$mail->AddAddress('info@soderkyl.se', 'Söderkyl Serviceformulär');
		}
